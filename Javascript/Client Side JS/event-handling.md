...
আপনি `addEventListener`-এর যে অ্যাডভান্সড বিষয়গুলো উল্লেখ করেছেন, সেগুলোর প্রত্যেকটির কার্যকর কোড উদাহরণ নিচে দেওয়া হলো। এই উদাহরণগুলো আপনাকে প্র্যাকটিক্যাল প্রজেক্টে এই কনসেপ্টগুলো ব্যবহার করতে সাহায্য করবে।

### ৪. অ্যানোনিমাস ফাংশন (Anonymous Function)
...
### ৯. DOM লোড হওয়ার পর লিসেনার যোগ করা
...
```
ইভেন্ট লিসেনারকে একবার ব্যবহার করার পর অটোমেটিক রিমুভ করার জন্য আমরা `{ once: true }` অপশনটি ব্যবহার করি। এটি কোডকে অনেক ক্লিন রাখে কারণ আপনাকে ম্যানুয়ালি `removeEventListener` কল করতে হয় না।

নিচে এর কোড এবং কাজ করার প্রক্রিয়া দেওয়া হলো:

### ১১. Once Listener (একবার কার্যকর হওয়া)

যখন আপনি চান কোনো বাটন বা ইভেন্ট শুধু প্রথম ক্লিকের জন্যই কাজ করবে, তখন এটি ব্যবহার করা হয়।

```javascript
const btn = document.querySelector('#submitBtn');

btn.addEventListener('click', () => {
    console.log("এই মেসেজটি শুধুমাত্র একবারই দেখা যাবে!");
}, { once: true }); 
// একবার ক্লিক করার পর ব্রাউজার নিজে থেকেই লিসেনারটি মেমোরি থেকে মুছে ফেলবে।

```

### ইভেন্ট লিসেনার ম্যানেজমেন্টের সারসংক্ষেপ

ইভেন্ট লিসেনার কিভাবে কাজ করে (Capturing vs Bubbling) তা বুঝতে নিচের টেবিলটি আপনাকে সাহায্য করবে:

| ফিচার | সিনট্যাক্স | উদ্দেশ্য |
   |
| **Once** | `{ once: true }` | ইভেন্টটি একবার ঘটার পর লিসেনার রিমুভ করে দেয়। |
| **Passive** | `{ passive: true }` | স্ক্রল পারফরম্যান্স স্মুথ করে (`preventDefault` কল করা যায় না)। |
| **Capture** | `{ capture: true }` | ইভেন্টটি বাবলিং হওয়ার আগেই (উপর থেকে নিচে নামার সময়) ধরে ফেলে। |

### প্রো টিপ: AbortController দিয়ে লিসেনার রিমুভ করা

বর্তমানে অনেকগুলো ইভেন্ট লিসেনার একসাথে রিমুভ করার জন্য `AbortController` ব্যবহার করা হয়, যা কোড ম্যানেজমেন্ট সহজ করে।

```javascript
const controller = new AbortController();

// লিসেনার অ্যাড করার সময় সিগন্যাল পাস করা
...
// যখনই প্রয়োজন হবে সব লিসেনার একসাথে বন্ধ করতে:
controller.abort(); 
...
এতগুলো উদাহরণ দেখার পর এখন হাতেনাতে কাজ করার পালা। আপনি কি চান আমি **"Event Delegation"** এবং **"Local Storage"** ব্যবহার করে একটি সম্পূর্ণ **"Shopping Cart"** প্রজেক্ট তৈরি করে দেখাই যেখানে পেজ রিফ্রেশ দিলেও ডেটা থাকবে?